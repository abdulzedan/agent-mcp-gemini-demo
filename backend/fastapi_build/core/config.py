...
env_path = Path(__file__).resolve().parents[2] / ".env"  # Moves up two levels from core/ to backend/
...
if env_path.exists():
    ...
    print(f".env file not found at {env_path}. Relying on environment variables directly.")
...
class Settings(BaseSettings):
    ...
    ADK_GEMINI_MODEL: str = "gemini-2.0-flash"  # Sticking to ADK docs recommendation for Google Search etc.

    # Default path for the MCP server script (relative to backend directory)
    # This will be used by StdioServerParameters
    YOUTUBE_MCP_SERVER_SCRIPT_PATH: str = "fastapi_build/mcp_servers/youtube_transcript_mcp_server.py"
...
try:
    ...
    print(f"Google Auth Default error: {e}. Ensure ADC or service account is configured.")
